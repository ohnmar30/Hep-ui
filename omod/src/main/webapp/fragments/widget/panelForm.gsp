...
<div class="ke-panel-controls">
	...
	<button type="submit" form="${ id }">
		<img src="${ ui.resourceLink("chaiui", "images/glyphs/ok.png") }" /> ${ config.submitLabel }
	</button>
	<% } %>

	<% if (config.cancelLabel) { %>
	<button type="button" onclick="publish('${ id }.reset'); ${ config.cancelFunction }()">
		<img src="${ ui.resourceLink("chaiui", "images/glyphs/cancel.png") }" /> ${ config.cancelLabel }
	</button>
	<% } %>
</div>
<% } %>
...
<script type="text/javascript">
	jq(function() {
		jq('#${ id }').submit(function(e) {
			e.preventDefault();
			chaiui.clearFormErrors('${ id }');

			<% if (config.submitLoadingMessage) { %>
			chaiui.openLoadingDialog({ message: '${ ui.escapeJs(config.submitLoadingMessage) }' });
			<% } %>
			var form = jq(this);
			...
			jq.ajax({ type: "POST", url: form.attr('action'), data: data, dataType: "json" })
					...
					.error(function(jqXHR, textStatus, errorThrown) {
						chaiui.showFormErrors('${ id }', jqXHR.responseText);
						<% if (config.submitLoadingMessage) { %>
						chaiui.closeDialog();
						<% } %>
					});
