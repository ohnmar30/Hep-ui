<%
	...
	def formatValue = { val ->
		if (val instanceof Date) {
			return config.showTime ? chaiui.formatDateTime(val): chaiui.formatDate(val)
		}
	...
	if (config.extra instanceof Date) {
		formattedExtra = chaiui.formatDate(config.extra)
	}
	else if (config.value instanceof Date && config.showDateInterval) {
		formattedExtra = chaiui.formatInterval(config.value)
	}
%>
