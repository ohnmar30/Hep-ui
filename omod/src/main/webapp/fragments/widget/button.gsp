...
<div id="${ id }" class="${ cssClass }">
	<% if (config.icon && config.iconProvider) { %>
		${ ui.includeFragment("chaiui", "widget/icon", [ iconProvider: config.iconProvider, icon: config.icon, iconOverlay: config.iconOverlay, iconOverlayProvider: config.iconOverlayProvider ]) }
	<% } %>
