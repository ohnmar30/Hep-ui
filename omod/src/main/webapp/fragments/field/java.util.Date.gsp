...
<script type="text/javascript">
	jq(function() {
		...
		jq('#${ config.id }_date, #${ config.id }_hour, #${ config.id }_minute').change(function() {
			chaiui.updateDateTimeFromDisplay('${ config.id }', ${ config.showTime ? "true" : "false" });
		});
	});
</script>
<% if (config.modelName) { %>
...
<input id="${ config.id }_date" type="text" <% if (initialDatetime) { %>value="${ chaiui.formatDate(initialDatetime) }"<% } %>/>
<% if (config.showTime) { %>
