...
%>
${ ui.includeFragment("chaiui", "widget/search", config.mergeAttributes([ searchType: "concept", searchParams: searchParams ])) }
...
<script type="text/javascript">
	jq(function() {
		...
		subscribe('${ config.parentFormId }.reset', function() {
			chaiui.setSearchField('${ config.id }', jq('#${ config.id }').data('default-value'));
		});
