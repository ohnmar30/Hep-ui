/**
 * ChaiUI main javascript library
 */
 ...
 * Configure AngularJS
 */
angular.module('chaiui', ['ngAnimate', 'ui.bootstrap'])
	...
	.filter('keDateAuto', function($filter) {
		return function(input) {
			return chaiui.isToday(new Date(input)) ? $filter('keTime')(input) : $filter('keDate')(input);
		};
	})
...
jQuery(function() {
	...
	jQuery('.ke-search').each(function() {
		...
		var searchConfig = chaiui.getSearchConfig(searchType);

		if (searchParams) {
			searchParams = chaiui.deparam(searchParams.replace('&amp;', '&'));
		}
...
(function(chaiui, $) {
	...
	function createButton(id, label, icon) {
		...
		if (icon) {
			html += '<img src="' + ui.resourceLink('chaiui', 'images/glyphs/' + icon + '.png') + '" /> '
		}
		html += label + '</button>';
		return html;
	}
	 ...
	 */
	chaiui.updateController = function(controllerId, update) {
	 ...
	 */
	chaiui.setupAjaxPost = function(formId, options) {
		...
		$('#' + formId).submit(function(event) {
			...
			$.post(form.attr('action'), form.serialize(), null, 'json')
				...
				.fail(function(xhr) {
					chaiui.showFormErrors(formId, xhr.responseText);
				})
				.always(function() {
	 ...
	 */
	chaiui.openLoadingDialog = function(options) {
		var defaults = { heading: null, message: null };
		var options = options ? $.extend(defaults, options) : defaults;

		var html = '<div class="ke-panel-content" style="text-align: center; padding: 10px"><img src="' + ui.resourceLink('chaiui', 'images/loading.gif') + '"/>';
		if (options.message) {
			html += '<br /><br />' + options.message;
		}
		html += '</div>';

		chaiui.openPanelDialog({ heading: options.heading, content: html, width: 40, height: 20 });
	};
	 ...
	 * @param options the options
	 */
	chaiui.openConfirmDialog = function(options) {
		var defaults = {
		...
		var okButtonId = chaiui.generateId();
		var cancelButtonId = chaiui.generateId();
		...
		$('#' + okButtonId).click(function() { options.okCallback(); chaiui.closeDialog(); });
		$('#' + cancelButtonId).click(function() { options.cancelCallback(); chaiui.closeDialog(); });
	};
	 ...
	 * @param options the options
	 */
	chaiui.openAlertDialog = function(options) {
		var defaults = {
			heading: null, message: '',
	 ...
	 * @param options the options
	 */
	chaiui.openDynamicDialog = function(options) {
		var defaults = { heading: null, width: null, height: null, scrolling: false };
		options = options ? $.extend(defaults, options) : defaults;

		var tmpContent = '<div class="ke-panel-content" style="text-align: center; padding: 10px"><img src="' + ui.resourceLink('chaiui', 'images/loading.gif') + '"/>';

		chaiui.openPanelDialog({ heading: options.heading, content: tmpContent, width: options.width, height: options.height });
	 ...
	 * @param options the options
	 */
	chaiui.openPanelDialog = function(options) {
		var defaults = { heading: null, width: null, height: null, scrolling: false };
		options = options ? $.extend(defaults, options) : defaults;
	 ...
	 */
	chaiui.optimizeModalContent = function() {
	 ...
	 */
	chaiui.updateDateTimeFromDisplay = function(fieldId, hasTime) {
		chaiui.clearFieldErrors(fieldId); // clear field errors
		var field = $('#' + fieldId);
		...
		try {
			...
			chaiui.showFieldError(fieldId, 'Invalid date');
		}
	};
	 ...
	 */
	chaiui.notifySuccess = function(message) {
		if (message) {
	 ...
	 * @param message the message
	 */
	chaiui.notifyError = function(message) {
		if (message) {
	 ...
	 */
	chaiui.configureSearch = function(searchType, config) {
	 ...
	 * @returns {*}
	 */
	chaiui.getSearchConfig = function(searchType) {
	 ...
	 */
	chaiui.showFormErrors = function(formId, response) {
		var form = $('#' + formId);
		...
		try {
			...
			for (key in err.fieldErrors) {
				...
				if (fieldId && chaiui.hasErrorField(fieldId)) {
					chaiui.showFieldError(fieldId, errorMsg);
				}
				else {
			...
			chaiui.notifyError("Failed " + ex + " (" + response + ")");
		}
	};
	 ...
	 * @param formId the form id
	 */
	chaiui.clearFormErrors = function(formId) {
	 ...
	 */
	chaiui.hasErrorField = function(fieldId) {
	 ...
	 */
	chaiui.showFieldError = function(fieldId, message) {
	 ...
	 * @param fieldId the field id
	 */
	chaiui.clearFieldErrors = function(fieldId) {
	 ...
	 * @param value
	 */
	chaiui.setSearchField = function(fieldId, value) {
	 ...
	 */
	chaiui.setDateField = function(fieldId, value) {
	 ...
	 * @param value the value to check
	 */
	chaiui.setRadioField = function(fieldId, value) {
	 ...
	 * @param fieldId the field id
	 */
	chaiui.validateRequiredField = function(fieldId) {
		if (($('#' + fieldId).val() + '').length == 0) {
			chaiui.showFieldError(fieldId, 'Required');
		}
	};
	 ...
	 * @param fieldId the field id
	 */
	chaiui.validateIntegerField = function(fieldId) {
		if (!isValidInteger($('#' + fieldId).val())) {
			chaiui.showFieldError(fieldId, 'Not an integer');
		}
	};
	 ...
	 * @returns {boolean}
	 */
	chaiui.isToday = function(date) {
		var today = new Date();
		return date.getDate() == today.getDate() && date.getMonth() == today.getMonth() && date.getFullYear() == today.getFullYear();
	}


}( window.chaiui = window.chaiui || {}, jQuery ));