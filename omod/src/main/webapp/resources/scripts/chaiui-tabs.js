...
(function(chaitab, $) {
	...
	$(function() {
		$('.ke-tabs').each(function() {
			...
			chaitab.activateTab(tabs.attr('id'), firstTabId);
			...
			tabs.find('.ke-tabmenu-item').click(function() {
				...
				chaitab.activateTab(tabs.attr('id'), tabId);
			});
		});
	});
	 ...
	 */
	chaitab.activateTab = function(tabsId, tabId) {
		var tabs = $('#' + tabsId);
...
}( window.chaitab = window.chaitab || {}, jQuery ));